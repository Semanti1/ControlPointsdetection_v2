...
net = NetLF()
net.load_state_dict(torch.load(r'C:\Users\Semanti Basu\Documents\OneDrive_2020-02-19\3D Ceaser dataset\Image and point generation\Image and point generation\frontaltrainedmodel_leftfoot_multifc_100epochs_first100_fc4.pth'))
net1 = Net()
net1.load_state_dict(torch.load(r'C:\Users\Semanti Basu\Documents\OneDrive_2020-02-19\3D Ceaser dataset\Image and point generation\Image and point generation\frontaltrainedmodel_first100_lr0001_100epochs.pth'))
## print out your net and prepare it for testing (uncomment the line below)
net2 = NetLL()
net2.load_state_dict(torch.load(r'C:\Users\Semanti Basu\Documents\OneDrive_2020-02-19\3D Ceaser dataset\Image and point generation\Image and point generation\frontaltrainedmodel_legs_200epochs_first100.pth'))
net.eval()
net1.eval()
net2.eval()
net3 = NetRF()
net3.load_state_dict(torch.load(r'C:\Users\Semanti Basu\Documents\OneDrive_2020-02-19\3D Ceaser dataset\Image and point generation\Image and point generation\frontaltrainedmodel_rightfoot_100epochs_first100.pth'))
net3.eval()
data_transform = transforms.Compose([#Rescale(429),
                                     RandomCrop(429),
                                     Normalize(),
                                     ToTensor()])
transformed_dataset = FacialKeypointsDataset(csv_file=r'C:\Users\Semanti Basu\Documents\OneDrive_2020-02-19\3D Ceaser dataset\Image and point generation\Image and point generation\frontalpoints_testing_last700.csv',
                                             root_dir=r'C:\Users\Semanti Basu\Documents\OneDrive_2020-02-19\3D Ceaser dataset\Image and point generation\Image and point generation\ceasar_mat_test_last700',
...
def net_sample_output():
    ...
    for i, sample in enumerate(test_loader):
        ...
        #key_pts = key_pts[:, 14:43]
        # convert images to FloatTensors
        images = images.type(torch.FloatTensor)
        #print(sample['name'])
        # forward pass to get net output
        #output_pts = net2(images)
        #output_pts = output_pts.view(output_pts.size()[0], 29, -1)
        ...
        out_wholebod[:, 14:43] = (1.0 / 2) * (out_wholebod[:, 14:43] + out_leg)'''
        #print(output_pts)
        visualize_output(images, out_wholebod, sample['name'], key_pts)
        # break after first image is tested
        #if i == 0:
            #return
            #return images, output_pts, key_pts, sample['name']
...
def visualize_output(test_images, test_outputs, name, gt_pts=None, onlyfoot=None, batch_size=1):
    #with open("test_write_to_csv.csv", "w", newline='') as f:
        #writer = csv.writer(f)
    for i in range(batch_size):
        ...
        row = np.append(basename(normpath(name[i])),predicted_key_pts.flatten())
        writer.writerow(row)
...
'''test_images, test_outputs, gt_pts, name = net_sample_output()
...
# call it
visualize_output(test_images, test_outputs, name, gt_pts)'''
with open("test_write_to_csv_frontal_last700.csv", "w", newline='') as f:
    writer = csv.writer(f)
    net_sample_output()